...
def log_retweeted_urls(successfully_retweeted_urls):
  ...
  for post_url in successfully_retweeted_urls:
    ...
    count += 1
  db.session.commit()
  print(f"Committed {count} tweets to the db.")
  return True
...
def send_retweets(cleaned_tweets_list, tweepy_client):
  ...
  for tweet in cleaned_tweets_list:
    try:
      ...
      continue
  print(f"Retweeted {len(successfully_retweeted_urls)} Sigle stories.")
  return successfully_retweeted_urls

def remove_older_tweets(days_old, model, db):
  count = 0
  old_datetime = datetime.now() - timedelta(days=days_old)
  all_tweets = model.query.all()
  for tweet in all_tweets:
    if tweet.date_retweeted <= old_datetime:
      count += 1
      db.session.delete(tweet)
  db.session.commit()
  print(f"Deleted {count} old tweets from the database.")
  return True
...
def activate_siglebot():
  ...
  print(f"Twitter URL response looks like: {downloaded_tweets[0].post_url[0]}")
  ...
  remove_older_tweets(14, RetweetedTweet, db)
  return True
